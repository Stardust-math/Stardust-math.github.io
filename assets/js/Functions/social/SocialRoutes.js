(function () {
  ...
  function getSocialBaseUrl() {
    return new URL('social/', document.baseURI);
  }

  function getRoute(view) {
    const normalized = normalizeView(view) || DEFAULT_VIEW;
    return new URL(VIEW_TO_SLUG[normalized] + '/', getSocialBaseUrl()).pathname;
  }
  ...
  function getSocialPathParts(pathname) {
    ...
    return {
      parts,
      socialIndex,
      slug: socialIndex >= 0 ? parts[socialIndex + 1] : ''
    };
  }

  function resolveViewFromPath(pathname) {
    const info = getSocialPathParts(pathname);
  ...
  function toRouteLink(control) {
    ...
    if (control.tagName && control.tagName.toLowerCase() === 'a') {
      ...
      applySocialSubnavCursor(control);
      return control;
    }
  ...
  function enhanceSocialSubnav() {
    document.querySelectorAll('.social-switcher .social-switch-btn').forEach((control) => {
  ...
  function getSocialSetter() {
    if (window.SocialShell && typeof window.SocialShell.setSocialView === 'function') {
      return window.SocialShell.setSocialView.bind(window.SocialShell);
    }
  ...
  function handleSocialSubnavClick(event) {
    const control = event.target && event.target.closest
      ? event.target.closest('.social-switcher .social-switch-btn')
      : null;

    if (!control) return;

    const view = normalizeView(control.dataset ? control.dataset.view : '');
    if (!view) return;
  ...
  function handleSocialSubnavKeydown(event) {
    ...
    const buttons = Array.from(root.querySelectorAll('.social-switcher .social-switch-btn'));
    ...
    activateView(next.dataset ? next.dataset.view : DEFAULT_VIEW, {
      updateHistory: true
    });
  }

  document.addEventListener('click', handleSocialSubnavClick, true);
  document.addEventListener('keydown', handleSocialSubnavKeydown, true);

  window.addEventListener('popstate', () => {
    window.setTimeout(() => {
      if (isSocialPath(window.location.pathname)) {
        enterFromLocation();
      }
    }, 0);
  });
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', enhanceSocialSubnav);
  } else {
    enhanceSocialSubnav();
  }
