(function () {
  ...
  function bindOrcidQrModal(root) {
    const socialRoot = root || document.getElementById('social');
    if (!socialRoot || socialRoot.dataset.orcidQrBound === 'true') return;

    const openBtn = socialRoot.querySelector('[data-orcid-qr-open]');
    const modal = socialRoot.querySelector('#orcid-qr-modal');
    const dialog = socialRoot.querySelector('.orcid-qr-dialog');
    const closeBtns = socialRoot.querySelectorAll('[data-orcid-qr-close]');
  ...
  function ensureModal(root) {
    ...
    let modal = socialRoot.querySelector('#orcid-qr-modal');

    if (!modal) {
      socialRoot.insertAdjacentHTML('beforeend', renderModalHTML());
      modal = socialRoot.querySelector('#orcid-qr-modal');
    }
